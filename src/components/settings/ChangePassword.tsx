...
export function ChangePassword(): JSX.Element {
  const { usePostChangePassword } = useRepositories();
  const { modal, setModalState, setAlertState } = useBoundStore();
  const { changePassword } = modal;

  const currentPassword = ''; //  get current password somehow to
  ...
  const { mutateAsync } = usePostChangePassword();
  const data = useGetQueryData<AuthResponse>([RQ_KEY.USER_DATA]);

  const handleClick = async () => {
    setIsLoading(true);
    try {
      await mutateAsync({ password: { old: password, new: newPassword }, id: data?.user.id });
  ...
  const handleNewPassword = (e: any) => {
    ...
    if (newRepassword !== '' && e.target.value !== newRepassword) {
      setIsPasswordNotMatch(true);
    } else {
      setIsPasswordNotMatch(false);
    }
  ...
  const handleNewRepassword = (e: any) => {
        ...
        <>
          <div ref={modalRef} className="fixed inset-0 z-50 bg-[#111827] bg-opacity-100">
            ...
            <div className="mx-5">
                ...
                <div className="mt-12">
                  ...
                  <div className="relative mt-3">
                    <input
                      ...
                      name="password"
                      value={password}
                      onFocus={() => setIsPasswordWrong(false)}
                      className={`${
                        isPasswordWrong ? 'border-2 border-red-600' : 'border'
                    ...
                    />
                    {isPasswordWrong && <p className="m-2 text-[10px] text-red-600">Invalid Password</p>}
                    <button
                      onClick={e => {
                ...
                <div className="mt-8">
                  ...
                  <div className="relative mt-3">
                    <input
                      ...
                      className={`border h-[46px] w-full rounded-md bg-gray-800 px-6 text-sm text-gray-200 shadow-sm ${
                        isPasswordShort ? 'border-red-600' : ''
                      } ${isPasswordShort ? 'focus:outline-red-600' : 'focus:outline-blue-600'}`}
                      value={newPassword}
                    />
                    {isPasswordShort && (
                ...
                <div className="mt-8">
                  ...
                  <div className="relative mt-3">
                    <input
                      ...
                      name="password"
                      className={`border h-[46px] w-full rounded-md bg-gray-800 px-6 text-sm text-gray-200 shadow-sm ${
                        isPasswordNotMatch ? 'border-red-600' : ''
                      } ${isPasswordNotMatch ? 'focus:outline-red-600' : 'focus:outline-blue-600'}`}
                      value={newRepassword}
                    />
                    <>
                      {isPasswordNotMatch && (
              ...
              <div className="mt-10">
                {/* <button className="w-full rounded-md bg-blue-600 py-3" >
                  Change
                </button> */}
                <button onClick={handleClick} className="relative mt-4 w-full rounded-md bg-blue-600 py-3" style={{ height: '46px' }}>
                  {isLoading ? (
                    <div className="loader absolute top-0 left-0 right-0 bottom-0 flex justify-center items-center">
                      <img src={loading} alt="" />
                    </div>
                  ) : (
                    'Change'
